...
# 텍스트 붙여넣기 파싱 처리 함수
# ---------------------------------------------------------------------------
def handle_paste_parse(pasted_text: str):
    """붙여넣기된 텍스트를 파싱하고 세션 상태를 업데이트합니다."""
    ...
    try:
        ...
        # paste_counter 증가 및 소스 이름 생성
        start_counter = st.session_state.get('paste_counter', 0)

        for idx, block in enumerate(blocks):
            counter = start_counter + idx + 1
            source_name = f"데이터 {counter}"
        ...
        # 카운터 업데이트
        st.session_state['paste_counter'] = start_counter + len(blocks)
...
if app_mode == 'basic':
    ...
    # =======================================================================
    # 고급 모드: 전체 기능 (텍스트 붙여넣기 + 파일 업로드 + 샘플 + 누적)
    # =======================================================================
    ...
    if st.session_state['measurement_data'] is not None:
        ...
        if st.session_state['cumulative_data'] is not None and len(st.session_state['cumulative_data']) > 0:
            ...
            # 소스별 샘플 수 표시
            if 'SOURCE_FILE' in cumul_df.columns:
                source_counts = cumul_df['SOURCE_FILE'].value_counts().sort_index()
                source_summary = ", ".join(
                    f"{src}: {cnt}샘플" for src, cnt in source_counts.items()
                )
                st.info(f"누적 데이터: 총 {len(cumul_df)}개 샘플 ({source_summary})")
            else:
